...
var empty_thumbnail = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
...
window.addEventListener('message', function (e) {
  var activeIframe = selected_relation ? document.querySelector(selected_relation.iframe_selector) : null;
...
function getSelectedIdsFromInput(form_relation) {
  var input = document.querySelector(form_relation.input_selector);
  if (!input) {
    return [];
  }
  return String(input.value || '').split(',').map(function (item) {
    return String(item || '').trim();
  }).filter(Boolean);
}
function buildImageMetadataMap(form_relation) {
  var map = {};
  form_relation.selected_images.forEach(function (item) {
    if (!item || !item.id) {
      return;
    }
    map[String(item.id)] = item;
  });
  var selector = "#".concat(form_relation.relationid, " .previously_selected_images li");
  document.querySelectorAll(selector).forEach(function (item) {
    var id = String(item.dataset.id || '');
    if (!id || map[id]) {
      return;
    }
    map[id] = _objectSpread({}, item.dataset);
  });
  return map;
}
function syncSelectedImagesFromInput(form_relation) {
  var ids = getSelectedIdsFromInput(form_relation);
  var metadata = buildImageMetadataMap(form_relation);
  form_relation.selected_images = ids.map(function (id) {
