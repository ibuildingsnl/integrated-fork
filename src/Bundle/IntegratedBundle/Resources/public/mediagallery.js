...
var EMBEDDED_IMAGE_EDITOR_CANCEL = 'integrated-media-image-editor-cancel';
var EMBEDDED_IMAGE_EDITOR_SAVED = 'integrated-media-image-editor-saved';
...
function bindUploadActions() {
  var doc = $(document);
  var hasFilesInDragEvent = function hasFilesInDragEvent(event) {
    var dragEvent = event && (event.originalEvent || event);
    var dataTransfer = dragEvent && dragEvent.dataTransfer;
    if (!dataTransfer) {
      return false;
    }
  ...
  var showUploadViewOnFileDrag = function showUploadViewOnFileDrag(event) {
  ...
  doc.off('dragenter' + MEDIA_GALLERY_NS, '.media-library').on('dragenter' + MEDIA_GALLERY_NS, '.media-library', showUploadViewOnFileDrag);
  doc.off('dragover' + MEDIA_GALLERY_NS, '.media-library').on('dragover' + MEDIA_GALLERY_NS, '.media-library', function (event) {
    if (hasFilesInDragEvent(event)) {
      event.preventDefault();
    }
  });
  doc.off('drop' + MEDIA_GALLERY_NS, '.media-library').on('drop' + MEDIA_GALLERY_NS, '.media-library', function (event) {
    if (hasFilesInDragEvent(event)) {
      event.preventDefault();
    }
  });
...
function buildEmbeddedImageEditorUrl(mediaId) {
  var _getEditPanelElements = getEditPanelElements(),
    wrapper = _getEditPanelElements.wrapper;
  if (!wrapper) {
    return '';
  }
  var basePath = String(wrapper.dataset.editimageiframepath || '').trim();
  var embeddedQuery = String(wrapper.dataset.editImageEmbeddedQuery || '').trim();
  if (!basePath) {
    return '';
  }
  var resolvedPath = basePath.replace('REPLACE', mediaId);
  if (!embeddedQuery) {
    return resolvedPath;
  }
  var separator = resolvedPath.includes('?') ? '&' : '?';
  return resolvedPath + separator + embeddedQuery;
}
function showEditPanelDetailsMode() {
  var _getEditPanelElements2 = getEditPanelElements(),
    panelContainer = _getEditPanelElements2.panelContainer,
    panel = _getEditPanelElements2.panel,
    editorFrame = _getEditPanelElements2.editorFrame;
...
function resetEmbeddedImageEditorState() {
  var _getEditPanelElements3 = getEditPanelElements(),
    panelContainer = _getEditPanelElements3.panelContainer,
    editorFrame = _getEditPanelElements3.editorFrame;
...
function openEmbeddedImageEditor(mediaId, selectedModus) {
  var _getEditPanelElements4 = getEditPanelElements(),
    panelContainer = _getEditPanelElements4.panelContainer,
    panel = _getEditPanelElements4.panel,
    editorFrame = _getEditPanelElements4.editorFrame;
  if (!panelContainer || !panel || !editorFrame) {
    return;
  }
  if (selectedModus !== 'media_gallery') {
...
function handleEmbeddedImageEditorMessage(event) {
  var _getEditPanelElements5 = getEditPanelElements(),
    editorFrame = _getEditPanelElements5.editorFrame;
  if (!editorFrame || !editorFrame.contentWindow || event.source !== editorFrame.contentWindow) {
    return;
  }
  var payload = event.data;
  if (!payload || _typeof(payload) !== 'object' || !('type' in payload)) {
    return;
  }
...
function syncSelectionStateFromDOM() {
  var selectedNodes = Array.from(document.querySelectorAll('.media-item.selected'));
  var selectedIds = selectedNodes.map(function (node) {
    return node.getAttribute('data-id');
  }).filter(Boolean);
  if (modi[selected_modus].selectOnlyOneEnabled) {
    var selectedId = selectedIds.length > 0 ? selectedIds[selectedIds.length - 1] : null;
...
function handleBulkItemClick(event) {
  if (modi[selected_modus].selectOnlyOneEnabled) {
    ...
    document.querySelectorAll('.media-item.selected').forEach(function (node) {
