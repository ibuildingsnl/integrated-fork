...
function applyInputColorPreview(input, color) {
  ...
  var green = rgba[1];
  var blue = rgba[2];
  var hexColor = formatHexColorValue(color);
...
function formatHexColorValue(color) {
  var hexParts = color.toHEXA();
  var red = String(hexParts[0] || '00').toUpperCase();
  var green = String(hexParts[1] || '00').toUpperCase();
  var blue = String(hexParts[2] || '00').toUpperCase();
  ...
  inputs.forEach(function (input) {
    ...
    pickr.on('save', function (color, instance) {
      ...
      if (selectedColor) {
        ...
        var syncedColor = syncPickrFromInputValue(input, instance);
