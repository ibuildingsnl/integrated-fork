...
class TwigColorTools extends AbstractExtension
{
    public function getFilters()
    {
        return [
            ...
            new TwigFilter('color_mix', [$this, 'color_mix']),
        ];
    }

    public const COLORPERCENT = 2.55;
    ...
    protected static function normalizeColor($color, &$mode = null)
    {
        ...
        if (str_contains($color, 'rgb')) {
            ...
            if (\strlen($color) == 3) {
                $color = str_repeat(substr($color, 0, 1), 2).str_repeat(substr($color, 1, 1), 2).str_repeat(
                    substr($color, 2, 1),
                    2
                );
            }
