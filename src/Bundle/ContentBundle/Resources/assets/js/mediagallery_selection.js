...
function getTypesUrl(types) {
    return types.reduce((accumulator, currentValue) => accumulator + 'available_contenttypes[]=' + currentValue.type + '&', '');
}
...
function addImageIDsToInputField() {
    $(selected_relation.input_selector).attr('value', JSON.stringify(selected_relation.selected_images.map(item => item.id)).replace('[', '').replace(']', '').replaceAll('"', ''))
}
