...
final class ChannelControllerCacheTtlTest extends TestCase
{
    ...
    public function testChannelsFragmentCacheKeyIncludesCurrentSessionId(): void
    {
        ...
        $cacheKey = $method->invoke($controller, $this->createUserStub('user-123'), $request, true, [
            ['id' => 'block-1', 'title' => 'Block 1', 'type' => 'text'],
        ]);

        self::assertSame('channels_'.md5(json_encode([
            'user' => 'user-123',
            'locale' => 'en',
            'session' => 'session-abc',
            'showBlocks' => true,
